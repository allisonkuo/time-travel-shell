	  ...
	  */
	  if (new_token->type == SUBSHELL || new_token->type == WORD)
	      ...
	      if(*(input++) == '\n' && new_token->type != HEAD)
		{
		  ...
		  new_stream->next = malloc(sizeof(token_stream));
		  new_stream = new_stream->next;
	...
	case ')':
	  ...
	  if (is_valid_word(*script))
	      ...
	      if (word == NULL)
		{
	      ...
	      while (count < input_size && is_valid_word(*script))
		{
		  word[word_counter] = *script;
		  word_counter++;
